...
function MaterialsList() {
    ...
    const fetchMaterials = async () => {
        try {
            setLoading(true);
            const response = await fetch('https://kass-checking-backend.vercel.app/api/v1/getall');
    ...
    const handleSaveMaterial = async (e) => {
        e.preventDefault();
        try {
            const url = editId ? `https://kass-checking-backend.vercel.app/api/v1/update/${editId}` : 'https://kass-checking-backend.vercel.app/api/v1/create';
    ...
    const handleDeleteMaterial = async (id) => {
        ...
        try {
            const response = await fetch(`https://kass-checking-backend.vercel.app/api/v1/delete/${id}`, { method: 'DELETE' });
    ...
    if (loading) return (
        <div className="layout-flex-wrapper">
            ...
            <style>{`
                ...
                .layout-flex-wrapper { display: flex; min-height: 100vh; background: #f8fafc; }
                .state-container { flex: 1; display: flex; flex-direction: column; justify-content: center; align-items: center; padding: 24px; text-align: center; }
                @media (max-width: 900px) { .layout-flex-wrapper { flex-direction: column; } }
                .spinner {
    ...
    if (error) return (
        <div className="layout-flex-wrapper">
            ...
            <style>{`
                html, body { overflow-x: hidden; max-width: 100%; }
                .layout-flex-wrapper { display: flex; min-height: 100vh; background: #f8fafc; }
                .state-container { flex: 1; display: flex; justify-content: center; align-items: center; padding: 24px; text-align: center; }
                @media (max-width: 900px) { .layout-flex-wrapper { flex-direction: column; } }
            `}</style>
